...
int main(int arg){
    ...
    float pos1[3] = {0, 0, 0};
    init_cube(&entities[0], pos1, 2.0f, COLOR_RED);
    float pos2[3] = {5, 0, 5};
    init_cube(&entities[1], pos2, 1.0f, COLOR_GREEN);
    float pos3[3] = {-5, 0, 5};
    init_cube(&entities[2], pos3, 1.0f, COLOR_BLUE);
	//////////////////////////////////////////////
	...
	while(1){
		hwbutts = SBHWButtons();	// get button presses
       	if(hwbutts & BTN_FIRE2) {	// pressed, exit back out to the OS
			exitCnt++;
			if(exitCnt>6){	// hold for 1 second (50fps)
				return(0x7);
			}
		...
		//update_camera(&cam, &input, 0.4f);
        render(&cam, entities, MAX_ENTITIES);
